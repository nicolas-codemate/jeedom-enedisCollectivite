...
class enedisCollectivite extends eqLogic {
    ...
    if ($this->getIsEnable() == 1) {
      ...
      foreach ($measureTypes as $measureType) {
        ...
        if (empty($_startDate) && $dailyCmd->getCollectDate() >= date('Y-m-d', strtotime('-1 day'))) {
          ...
          $data = $this->callEnedisCollectivite('/metering_data_d' . $measureType[0] . '/v5/daily_' . $measureType . '?start=' . $start_date . '&end=' . $end_date . '&usage_point_id=' . $usagePointId);
        ...
        if ($this->getConfiguration('no_load_curve', 0) != 1) {
          ...
          if (empty($_startDate) && $loadCmd->getCollectDate() >= date('Y-m-d')) {
            ...
            $data = $this->callEnedisCollectivite('/metering_data_' . $measureType[0] . 'lc/v5/' . $measureType . '_load_curve?start=' . $start_date_load . '&end=' . $end_date_load . '&usage_point_id=' . $usagePointId);
            if (isset($data['meter_reading']) && isset($data['meter_reading']['interval_reading'])) {
              foreach ($data['meter_reading']['interval_reading'] as $value) {
        ...
        if ($measureType == 'consumption') {
          ...
          if (empty($_startDate) && $dailyMaxCmd->getCollectDate() >= date('Y-m-d', strtotime('-1 day'))) {
            ...
            $data = $this->callEnedisCollectivite('/metering_data_dcmp/v5/daily_' . $measureType . '_max_power?start=' . $start_date . '&end=' . $end_date . '&usage_point_id=' . $usagePointId);
            if (isset($data['meter_reading']) && isset($data['meter_reading']['interval_reading'])) {
              foreach ($data['meter_reading']['interval_reading'] as $value) {
  ...
  public function obtainAuthToken() {
      // retrieve api_key from configuration
      // retrieve api_secret from configuration
      // generate token by base64 encoding api_key:api_secret
        $apiKey = config::byKey('configuration_apiKey','enedisCollectivite');
        $apiSecret = config::byKey('configuration_apiSecret','enedisCollectivite');
        if (empty($apiKey) || empty($apiSecret)) {
            throw new Exception(__('API Key or API Secret is not configured', __FILE__));
        }
        $token = base64_encode($apiKey . ':' . $apiSecret);

        // post the token to the Enedis API to obtain an access token
        ...
        cache::set('enedisCollectivite_access_token', $token, $result['expires_in'] ?? 3600); // cache default for 1 hour

      return $token;
  }
  
  public function callEnedisCollectivite($_path) {
    try {
      $url = config::byKey('service::cloud::url') . '/service/enedis2?path=' . urlencode($_path); // FIXME change URL.
      $request_http = new com_http($url);
